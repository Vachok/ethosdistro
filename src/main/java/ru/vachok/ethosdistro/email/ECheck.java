...
public static void setShouldOrFalse(boolean shouldOrFalse) {
    ...
    /**
     ...
     Вызывает проверку {@link #firstMBCheck()}. Если она падает в {@link MessagingException}
     или {@link NumberFormatException},
    ...
    /**
     ...
     <i>Если</i> сообщений нет и {@link #properties} не пусты - пытается взять
     {@link #stopMinutes} из {@link #properties}.<br>
     ...
     */
    private static int firstMBCheck() {
        FILE_PROPS.getProps();
    ...
    private static Message messageSubjectCheck(Message[] messages) {
        for(Message m : messages){
            try{
                ...
                if(m.getSubject().toLowerCase().contains("speed:")){
                    ...
                    messageToUser.info(SOURCE_CLASS, new TForms().fromArray(from), new TForms().fromEnum(allHeaders));
    ...
    private static void writeO() {
        try(OutputStream fileOutputStream = new FileOutputStream(OBJ_FILE);
            ...
            properties.setProperty("uptime",
                    ( float ) (TimeUnit
                                       .MILLISECONDS
                                       .toMinutes(System.currentTimeMillis() - ConstantsFor
                                               .START_TIME_IN_MILLIS) / 60) + "");
